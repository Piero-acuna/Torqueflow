...
import { useClients, useVehicles } from "../../services/clients.service";
...
export function NewOrderPage() {
  ...
  // Mecánicos y servicios desde Supabase Realtime
  const { data: rawMechanics } = useSupabaseCollection("mechanics", workshopId, {
    orderBy: { column: "name", ascending: true }
  });
  const { data: rawServices } = useSupabaseCollection("services", workshopId, {
    orderBy: { column: "name", ascending: true }
  });
  const mechanics = rawMechanics.filter((m) => m.active !== false);
  const services  = rawServices.filter((s)  => s.active !== false);

  const [form, setForm]                   = useState(() => ({ ...INITIAL_FORM, clientId: getQueryParam("client") }));
  const { data: clients = [] }            = useClients();
  const { data: vehicles = [] }           = useVehicles({ clientId: form.clientId });
  ...
  function toggleService(service) {
    setSelectedServices((current) => {
      ...
      return [...current, { serviceId: service.id, name: service.name, categoryId: service.category_id || "", quantity: 1, price: Number(service.price || 0) }];
    });
  }
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (files.length) {
        const photoEvidence = await uploadOrderPhotos(order.id, files, workshopId);
        await attachOrderPhotos(order.id, photoEvidence, workshopId);
      }

      showToast(`Orden ${order.orderNumber} creada correctamente.`);
  ...
  return (
    <>
      <PageHeader
        eyebrow="Recepción del vehículo"
        title="Nueva orden de trabajo"
        description="Completa el ingreso. Todos los datos se guardan en tiempo real."
        actions={<Button variant="ghost" type="button" onClick={() => navigate("/orders")}>Ver órdenes</Button>}
      />
      ...
      <form className="order-create-layout" onSubmit={submit}>
        <div className="order-create-main">
            ...
            {!clients.length ? (
              ...
              <div className="form-grid">
                <FormField label="Cliente" required>
                  <Select value={form.clientId} onChange={(event) => { updateField("clientId", event.target.value); updateField("vehicleId", ""); }}>
                    <option value="">Selecciona un cliente</option>
                    {clients.map((client) => <option key={client.id} value={client.id}>{client.name} · {client.phone}</option>)}
                  </Select>
                </FormField>
                <FormField label="Vehículo" required hint={!form.clientId ? "Selecciona primero el cliente." : ""}>
                  <Select value={form.vehicleId} disabled={!form.clientId} onChange={(event) => updateField("vehicleId", event.target.value)}>
                ...
                <FormField label="Nivel de combustible">
                  <div className="range-field">
                    <Input type="range" min="0" max="100" step="5" value={form.fuelLevel} onChange={(event) => updateField("fuelLevel", event.target.value)} />
                    <strong>{form.fuelLevel}%</strong>
                  </div>
                </FormField>
              </div>
            )}
          ...
          <SectionCard title="2. Diagnóstico y recepción" description="Deja constancia del estado en que ingresa el vehículo.">
            <div className="form-grid">
              ...
              <FormField label="Evidencias fotográficas" className="field--wide" hint="Imágenes JPG, PNG o WebP; se guardarán en Supabase Storage.">
            ...
            {selectedServices.length ? (
              <div className="selected-lines">
                {selectedServices.map((line, index) => (
                  <div key={`${line.serviceId}-${index}`} className="selected-line">
                    <span>{line.name}</span>
                    <strong>{formatMoney(line.price, workshop.currency)}</strong>
                    <button type="button" className="icon-button" onClick={() => setSelectedServices((current) => current.filter((_, itemIndex) => itemIndex !== index))}>×</button>
                  </div>
                ))}
              </div>
            ) : null}
          ...
          <SectionCard title="4. Planificación" description="Asigna responsable, prioridad y fechas comprometidas.">
            <div className="form-grid">
              <FormField label="Mecánico asignado">
                <Select value={form.mechanicId} onChange={(event) => updateField("mechanicId", event.target.value)}>
              ...
              <FormField label="Prioridad">
                <Select value={form.priority} onChange={(event) => updateField("priority", event.target.value)}>
                  {ORDER_PRIORITIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                </Select>
              </FormField>
        ...
        <aside className="quote-panel">
          ...
          {Number(form.budget || 0) > 0 && estimatedTotal > Number(form.budget) ? (
            <div className="form-alert form-alert--warning">La estimación supera el presupuesto autorizado.</div>
          ) : null}
          <div className="quote-panel__meta">
            <Badge tone={files.length ? "success" : "neutral"}>{files.length} fotos</Badge>
            <Badge tone={selectedServices.length ? "info" : "neutral"}>{selectedServices.length} servicios</Badge>
          </div>
          <Button type="submit" disabled={saving} className="button--full">{saving ? "Creando orden…" : "Crear orden de trabajo"}</Button>
        </aside>
      </form>
    </>
  );
